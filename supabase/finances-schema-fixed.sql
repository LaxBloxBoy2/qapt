-- Finances Module Schema (Fixed for existing database)
...
-- Financial Transactions (using property-based RLS like leases)
...
-- Transactions policies (using property-based RLS like leases)
CREATE POLICY "Users can view transactions for their properties" ON public.transactions
  FOR SELECT USING (
...
CREATE POLICY "Users can insert transactions for their properties" ON public.transactions
...
CREATE POLICY "Users can update transactions for their properties" ON public.transactions
  FOR UPDATE USING (
    property_id IS NULL OR 
    EXISTS (
      SELECT 1 FROM public.properties 
...
CREATE POLICY "Users can delete transactions for their properties" ON public.transactions
  FOR DELETE USING (
    property_id IS NULL OR 
    EXISTS (
      SELECT 1 FROM public.properties 
...
CREATE POLICY "Users can view attachments for their transactions" ON public.transaction_attachments
  FOR SELECT USING (
    EXISTS (
      ...
      AND (t.property_id IS NULL OR p.user_id = auth.uid())
    )
  );
