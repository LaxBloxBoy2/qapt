-- Alternative Tenants Schema for QAPT

-- Tenants Table with renamed foreign key column
CREATE TABLE IF NOT EXISTS public.tenants (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  auth_user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE, -- Renamed from user_id to auth_user_id
...
ON public.tenants
FOR SELECT
USING (auth.uid() = auth_user_id); -- Updated to use auth_user_id

-- Policy for inserting tenants (authenticated users can insert)
CREATE POLICY "Users can insert their own tenants"
ON public.tenants
FOR INSERT
WITH CHECK (auth.uid() = auth_user_id); -- Updated to use auth_user_id
