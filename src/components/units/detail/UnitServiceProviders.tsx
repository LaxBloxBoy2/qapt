...
import { UnitServiceProvider, unitServiceProviderSchema, unitServiceProviderTypes } from "@/types/unit";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { supabase } from "@/lib/supabase";

interface UnitServiceProvidersProps {
  unitId: string;
}
...
type UnitServiceProviderFormValues = z.infer<typeof unitServiceProviderSchema>;

export function UnitServiceProviders({ unitId }: UnitServiceProvidersProps) {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingProvider, setEditingProvider] = useState<UnitServiceProvider | null>(null);
  const { toast } = useToast();
  const queryClient = useQueryClient();

  // Fetch unit service providers
  const { data: providers, isLoading } = useQuery({
    queryKey: ["unit-service-providers", unitId],
    queryFn: async (): Promise<UnitServiceProvider[]> => {
      const { data, error } = await supabase
        .from("unit_service_providers")
        .select("*")
        .eq("unit_id", unitId)
  ...
  const addProvider = useMutation({
    mutationFn: async (provider: UnitServiceProviderFormValues) => {
      const { data, error } = await supabase
        .from("unit_service_providers")
        .insert([provider])
        .select()
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["unit-service-providers", unitId] });
      toast({
  ...
  const updateProvider = useMutation({
    mutationFn: async ({
      ...
      providerId: string;
      provider: UnitServiceProviderFormValues;
    }) => {
      const { data, error } = await supabase
        .from("unit_service_providers")
        .update(provider)
  ...
  const form = useForm<UnitServiceProviderFormValues>({
    resolver: zodResolver(unitServiceProviderSchema),
    defaultValues: {
      unit_id: unitId,
      type: "responsibility",
      category: "",
  ...
  const openAddDialog = (type: (typeof unitServiceProviderTypes)[number]) => {
  ...
  const openEditDialog = (provider: UnitServiceProvider) => {
    setEditingProvider(provider);
    form.reset({
      unit_id: unitId,
      type: provider.type as (typeof unitServiceProviderTypes)[number],
  ...
  const onSubmit = async (data: UnitServiceProviderFormValues) => {
    try {
      if (editingProvider) {
  ...
  const renderProvidersByType = (type: (typeof unitServiceProviderTypes)[number]) => {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-gray-500 dark:text-gray-400">
        Manage service providers and utilities for this unit.
      </p>
